...
# Plot a single snapshot (e.g., initial or final time step)
writevtk(joinpath(path, "initial_height"), (h0 = Y.h,))
writevtk(joinpath(path, "final_velocity"), (u_T = sol.u[end].u,))
...
ClimaCoreVTK.writepvd(joinpath(path, "height"), times, (h = Sol_h,)) # basis = :lagrange
ClimaCoreVTK.writepvd(joinpath(path, "velocity"), times, (u = Sol_u,))
ClimaCoreVTK.writepvd(joinpath(path, "vorticity"), times, (ω = Sol_vort,))
