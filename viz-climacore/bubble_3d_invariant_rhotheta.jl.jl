...
Δt = 0.01
T = 1.0
prob = ODEProblem(rhs_invariant!, Y, (0.0, T))
sol = solve(
    prob,
    SSPRK33(),
    dt = Δt,
...
Sol_theta =  Array{Fields.Field}(undef, length(times))
for t in 1:length(times)
    Sol_theta[t] = sol.u[t].Yc.ρθ ./ sol.u[t].Yc.ρ
end

ClimaCoreVTK.writepvd(joinpath(path, "theta"), times, (θ = Sol_theta,))
